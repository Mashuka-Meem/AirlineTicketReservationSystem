...
<% 	
	...
	Flight f1= FlightDao.getFlightRecordById1(Integer.parseInt(fromLoc), Integer.parseInt(toLoc), depDate);		
%>

 

<center>
<h4>Your One Way Flight's Total </h4><h1><%=f1.getTotalSeat()%></h1><h4> Seat Is Available</h4></center>
<h2>You Requested A Flight Of Following Information</h2>
	<form action="bookFlightAdminCode.jsp" method="POST">
		...
		<table>
			<tr>
				<td>Flight Number</td>
				<td><input type="text" name="planeName" value="<%=f1.getFlightNumber()%>" readonly=""></td>
			</tr>
			
			<tr>
				<td>Departure Date</td>
				<td><input type="Date" name="depart_date" value="<%=f1.getDepDate()%>" readonly=""></td>
			</tr>
			
			<tr>
				<td>Departure Time</td>
				<td><input type="Time" name="depart_time" value="<%=f1.getDepTime()%>" readonly=""></td>
			</tr>
			
			<tr>
				<td>Departure Location</td>
				<td><input type="text" name="fromLoc" value="<%=f1.getFrom()%>" readonly=""></td>			
		           
		           <sql:query dataSource = "${snapshot}" var = "result">
		           	SELECT `location`.`name` as `l`, `location`.`id` as `i` FROM `com.moshfiqrony`.`schedule` INNER JOIN `location` ON `schedule`.fromLoc = `location`.`id` WHERE `depDate` = 'depDate1' AND `fromLoc` = 'fromLoc1' AND `toLoc` = 'toLoc1';				
				</sql:query>
			...
			<tr>
				...
				<td><input type="text" name="toLoc" value="<%=f1.getTo()%>" readonly=""></td>	
		           
		           <sql:query dataSource = "${snapshot}" var = "result">
		           	SELECT `retlocation`.`name` as `r`, `retlocation`.`id` as `ir` FROM `com.moshfiqrony`.`schedule` INNER JOIN `retlocation` ON `schedule`.toLoc = `retlocation`.`id` WHERE  `depDate` = 'depDate1' AND  `fromLoc` = 'fromLoc1' AND `toLoc` = 'toLoc1';
				</sql:query>
				<c:forEach var = "row" items = "${result.rows}">
